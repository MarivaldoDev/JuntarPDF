...
class PDFMergerApp(ctk.CTk):
    ...
    def _build_interface(self):
        ...
        ctk.CTkLabel(
            header, text="FERRAMENTA DE DOCUMENTOS",
            text_color=self.colors["accent"],
            font=ctk.CTkFont(size=10, weight="bold"),
        ).grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(
            header, text="Juntar PDFs", text_color=self.colors["text"],
        ...
        ctk.CTkButton(
            toolbar, text="＋  Adicionar PDFs", command=self.add_files, height=38,
            fg_color=self.colors["accent"], hover_color=self.colors["accent_hover"],
            text_color="#101417", font=ctk.CTkFont(size=13, weight="bold"),
        ).pack(side="left")
        ctk.CTkButton(
            toolbar, text="Limpar lista", command=self.clear_files, height=36,
            fg_color="transparent", border_width=1, border_color=self.colors["border"],
            text_color=self.colors["muted"], hover_color=self.colors["surface_alt"],
        ).pack(side="right")
        ...
        self.file_list = ctk.CTkScrollableFrame(
            list_frame, label_text="  ARQUIVOS SELECIONADOS  ",
            label_text_color=self.colors["muted"], fg_color="transparent",
        ...
        self.empty_label = ctk.CTkLabel(
            ...
            text_color=self.colors["muted"], justify="center",
        ...
        self.status_label = ctk.CTkLabel(
            actions, text="0 arquivos", anchor="w", text_color=self.colors["muted"],
            font=ctk.CTkFont(size=12),
        )
        self.status_label.pack(side="left")
        ctk.CTkButton(
            actions, text="Mesclar e salvar", command=self.merge_files,
            width=190, height=42, state="disabled", corner_radius=10,
            fg_color=self.colors["surface_alt"], hover_color=self.colors["accent_hover"],
            text_color=self.colors["text"], font=ctk.CTkFont(size=13, weight="bold"),
        ).pack(side="right")
    ...
    def refresh_list(self):
        ...
        if not self.files:
            ...
            for index, filename in enumerate(self.files):
                row = ctk.CTkFrame(
                    self.file_list, fg_color=self.colors["surface_alt"], corner_radius=10,
                )
                row.pack(fill="x", padx=4, pady=3)
                ctk.CTkLabel(
                    row, text=f"{index + 1:02d}", width=42, anchor="center",
                    text_color=self.colors["accent"], font=ctk.CTkFont(size=12, weight="bold"),
                ).pack(side="left", padx=(4, 0))
                ctk.CTkLabel(row, text=Path(filename).name, anchor="w", text_color=self.colors["text"]).pack(
                    side="left", fill="x", expand=True
                )
                ctk.CTkButton(
                    row, text="↑", width=30, height=28, fg_color="transparent",
                    hover_color=self.colors["border"], text_color=self.colors["muted"],
                ...
                ctk.CTkButton(
                    row, text="×", width=30, height=28, fg_color="transparent",
                    hover_color="#573238", text_color=self.colors["danger"],
